...
struct ImportPlaylistSheet: View {
    ...
    var body: some View {
        ScrollView {
            ...
            if let fetchedPlaylistInfo = infoViewModel.fetchedPlaylistInfo {
                BetterAsyncImage(url: URL(string: fetchedPlaylistInfo.artwork), animated: false)
                    .scaledToFit()
                    .clipShape(RoundedRectangle(cornerRadius: 14))
                    .padding(.horizontal, 50)
                Text(fetchedPlaylistInfo.name)
                    .customFont(fontManager, .title, bold: true)
                if fetchedPlaylistInfo.description != "" {
                    Text(fetchedPlaylistInfo.description)
                        .customFont(fontManager, .headline)
                    Spacer()
                }
            }
            Button(action: {
                ...
                if recognizePlaylist(url: playlistURL).platform == .unknown {
                    ...
                    HStack {
                        ...
                        .customFont(fontManager, .title3)
                        .padding(EdgeInsets(top: 6, leading: 26, bottom: 6, trailing: 26))
                        .background(platformColor(platform: recognizePlaylist(url: playlistURL).platform))
                        .clipShape(RoundedRectangle(cornerRadius: 6))
                }
            }
                .buttonStyle(.plain)
