...
struct ThemePreview_feco: View {
    @Environment(PlayerManager.self) var playerManager
    @Environment(\.horizontalSizeClass) private var horizontalSizeClass
    @Environment(\.verticalSizeClass) private var verticalSizeClass
    @AppStorage("currentTheme") var currentTheme: String = "feco"
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            Button(action: {
                withAnimation {
                    currentTheme = "feco"
                    FontManager.shared.currentlyChosenTheme = .feco
                }
            }) {
                AlbumWideButton_faero(text: currentTheme == "feco" ? "Selected" : "Select", ArtworkID: "")
                    .frame(width: 200)
                    .background {
                        GeometryReader { g in
                            GlobalBackground_feco()
                ...
                .foregroundStyle(currentTheme == "feco" ? .secondary : .primary)
                .buttonStyle(.plain)
                .position(
